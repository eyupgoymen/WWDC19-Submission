...
public func testResult(ids: [Int]){
    if (ids[0] == 0 && ids[1] == 3) {
        PlaygroundPage.current.assessmentStatus = .pass(message: "Great job! We are done.")
        loadScene(pokemonID_1: pokemonID_1, pokemonID_2: pokemonID_2)
    } else {
        PlaygroundPage.current.assessmentStatus = .fail(hints: ["Try again!")
    }
}
